...
print '<script src="Scripts\findButton.js"></script>';
$thisPage = $_POST['page'];
$search = $_POST['search'];
...
$forNull = function($someVar, $isString = false, $killMe = false) {
	if (!isset($someVar)) {
		if ($killMe) { //while I actually don't like this implementation, it works with the way that I've set up my code. This is intended to redirect the page to 'Find' if there is no symbol found for the search query. Unfortunately, it has to load the page that one is searching and then redirect to Find with the correct search parameters.
			if($_POST['page'] == "Find")
			{
				return $_POST['search'] . " - No Data Found";
			}
			else {
				//reposts dir with find request
				print <<<REDIR
				<script>
				thisPage = "Find";
				fillContent();				
...
REDIR;
		...
		elseif ($isString) //pulls the search string from globals
			return strtoupper($_POST['search']); //this wouldn't work without the GLOBALS keyword...		
		else
			return "N/A";
	}
...
EODFind1;
	...
	{ //prints the result table
		print "<div id=\"findButton\" class=\"tablelayout\">";
		do {
			echo "
			...
			<div class=\"row\">
				...
				<p class=\"cell entry borderright\"><a class=\"quoteButtonA\" href=\"{$forNull(@$symSymbol)}\">Quote</a> <a class=\"histButtonA\" href=\"{$forNull(@$symSymbol)}\">History</a></p>
			</div>"; 
		} while($row = @$result->fetch_assoc() and extract($row));
		print "</div>";
	}
